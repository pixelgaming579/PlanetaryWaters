    ...
    public class RocketMoverMenu : MonoBehaviour
    {
        ...
        public void ChangePlanet()
        {
            ...
            if (PlayerController.main.player.Value as Rocket)
            {
                ...
                //     pos.Value = new Double2(0, planet.GetTerrainHeightAtAngle(0));
                // }
                pos.Value = new Double2(0, 0);
                newLocation.position = pos;
                // WorldTime.main.SetState(0, false, false);
                (PlayerController.main.player.Value as Rocket).physics.SetLocationAndState(newLocation.Value, false);
                if (planet.data.hasTerrain)
                {
                    pos.Value = new Double2(0, planet.Radius + planet.maxTerrainHeight);
                    newLocation.position = pos;
                }
                else
                {
                ...
                (PlayerController.main.player.Value as Rocket).physics.SetLocationAndState(newLocation.Value, true);
                // WorldTime.main.SetState(1, true, false);
            }
        }
    }
